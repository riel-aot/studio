...
import type { DashboardKpis, ReviewQueueItem, ReportListItem, StudentListItem, StudentListResponse } from '@/lib/events';
import { normalizeAssessmentIdentifier } from '@/lib/utils';
import { activityTracker } from '@/lib/activity-tracker';
import { FilePlus, PenSquare, AlertCircle, Activity, GraduationCap, CheckCircle2, Sparkles, MessageSquare, Lightbulb, Loader2, UserPlus, FileCheck2, FileEdit } from 'lucide-react';
...
function DashboardLoadingSkeleton() {
  return (
    <div className="space-y-6">
      <div className="h-40 w-full rounded-2xl bg-white dark:bg-[#111827] animate-pulse" />
...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    const handleActivityUpdate = () => setLocalActivity(activityTracker.get());
  ...
  const normalizedStudents = useMemo(() => {
    ...
    return (studentsData as any).students ?? (studentsData as any).data?.students ?? [];
  }, [studentsData]);

  const assessmentsThisMonth = useMemo(() => {
    const start = startOfMonth(new Date());
    return reports.filter(r => new Date(r.generatedAt) >= start).length;
  }, [reports]);
  ...
  useEffect(() => {
    ...
    const calculateClassPerformance = async () => {
        ...
        try {
          ...
          if (reportRequests.length > 0) {
            ...
            gradedReports = reportResponses.flatMap((response, index) => {
              const normalizedReport = (response as any)?.data?.report ?? (response as any)?.report ?? response;
      ...
      if (isMounted) setClassPerformance({ avgScore, completionRate: completedReports.length, masteryAchieved, criteriaBreakdown });
    };
  ...
  const { trigger: openReview } = useWebhook<{ assessmentId: string }, {}>({
    ...
    manual: true,
    onSuccess: (_, payload) => router.push(`/teacher/assessments/${normalizeAssessmentIdentifier(payload?.assessmentId) ?? payload?.assessmentId}`),
  });
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Welcome Banner */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#111827] to-[#1F2937] text-white p-6 md:p-8 shadow-xl border border-white/5">
        ...
        <div className="absolute right-4 bottom-0 h-40 w-64 pointer-events-none opacity-90 dark:opacity-80">
          <Image 
            src="/images/athena-classroom.png" 
            alt="Classroom illustration"
            fill
            className="object-contain scale-[1.6] origin-bottom-right translate-x-[20px] translate-y-[30px]"
            priority
      ...
      {/* KPIs Grid */}
      <div id="onboarding-kpis" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard 
          title="Grading Queue" 
                  ...
                  <TableBody>
                    {reviewQueueData.items.map((item) => (
                      <TableRow
                        ...
                        onClick={() => openReview({ assessmentId: item.assessmentId })}
                      ...
                      >
                        ...
                        <TableCell className="py-4 text-right pr-6">
                          <Badge variant={item.status === 'ai_draft_ready' ? 'default' : 'warning'} className="rounded-full px-2 text-[9px]">
                            {(item.status || 'needs_review').toUpperCase().replace('_', ' ')}
                          </Badge>
                        </TableCell>
                      </TableRow>
                    ))}
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card id="onboarding-quick-actions" className="bg-primary text-white border-none shadow-lg overflow-hidden relative rounded-2xl">
            ...
            <CardContent className="grid gap-2 relative z-10 px-5 pb-5">
              <Button size="sm" asChild className="w-full bg-white text-primary hover:bg-slate-50 h-11 font-bold rounded-xl transition-all border-none shadow-sm">
                <Link href="/teacher/assessments">
                  <FilePlus className="mr-2 h-4 w-4 stroke-[2.5]" /> Assignments
                </Link>
              </Button>
              <Button asChild size="sm" variant="outline" className="w-full bg-white/10 border-white/20 text-white hover:bg-white/20 h-11 font-bold rounded-xl transition-all">
                <Link href="/teacher/students">
                  <UserPlus className="mr-2 h-4 w-4 stroke-[2.5]" /> Add Student
                </Link>
              </Button>
            </CardContent>
          </Card>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <Card className="lg:col-span-8 border-border bg-white dark:bg-[#111827] shadow-lg overflow-hidden rounded-3xl self-start">
            <CardHeader className="bg-white dark:bg-[#111827] border-b border-border py-4 px-6 flex flex-row items-center justify-between">
              <div>
            ...
            <CardContent className="p-6">
              {classPerformanceLoading ? (
                <div className="h-[220px] flex items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-primary opacity-20" /></div>
                      ...
                      <BarChart data={classPerformance.criteriaBreakdown} margin={{ top: 25, bottom: 5 }}>
                        <defs>
                          <linearGradient id="barRedLight" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor="#EF4444" stopOpacity={0.1}/><stop offset="100%" stopColor="#EF4444" stopOpacity={1}/>
                          </linearGradient>
                          <linearGradient id="barOrangeLight" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor="#F59E0B" stopOpacity={0.1}/><stop offset="100%" stopColor="#F59E0B" stopOpacity={1}/>
                          </linearGradient>
                          <linearGradient id="barGreenLight" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor="#10B981" stopOpacity={0.1}/><stop offset="100%" stopColor="#10B981" stopOpacity={1}/>
                          </linearGradient>
                        </defs>
                        <XAxis dataKey="criterion" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10, fontWeight: 700 }} dy={5} />
                        ...
                        <Tooltip 
                          ...
                          content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                              ...
                              return (
                                <div className="bg-white dark:bg-[#111827] p-3 rounded-2xl shadow-2xl border border-border">
                                  <p className="text-[10px] font-bold uppercase text-primary mb-1">{d.criterion}</p>
                                  <p className="text-sm font-extrabold text-foreground">Level {formatProficiencyLevel(d.averageScore)} ({d.averageScore.toFixed(1)})</p>
                                  <p className="text-[9px] font-bold text-muted-foreground uppercase">From {d.count} assessments</p>
                                </div>
                              );
                            }
                            return null;
                        ...
                        />
                        <Bar dataKey="averageScore" barSize={40} shape={<BreathingBar />}>
                          {classPerformance.criteriaBreakdown.map((entry, idx) => (
                            <Cell key={`cell-${idx}`} fill={getLevelGradientId(formatProficiencyLevel(entry.averageScore))} />
                          ))}
                          <LabelList dataKey="averageScore" position="top" formatter={(v: number) => formatProficiencyLevel(v)} style={{ fill: '#64748b', fontSize: 11, fontWeight: 800 }} offset={12} />
                        </Bar>
                      </BarChart>
                    ) : (
                      <AreaChart data={classPerformance.criteriaBreakdown} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                        <defs><linearGradient id="colorProgress" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="hsl(var(--primary))" stopOpacity={0.3}/><stop offset="95%" stopColor="hsl(var(--primary))" stopOpacity={0}/></linearGradient></defs>
                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border) / 0.5)" />
                        <XAxis dataKey="criterion" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 9, fontWeight: 700 }} />
                        <YAxis domain={[0, 8]} axisLine={false} tickLine={false} tick={{ fill: '#cbd5e1', fontSize: 9 }} tickFormatter={(v) => formatProficiencyLevel(v)} />
                        <Tooltip content={({ active, payload }) => active && payload ? <div className="bg-white dark:bg-[#111827] p-2 rounded-xl border border-border shadow-lg"><p className="text-[9px] font-bold text-primary uppercase">{payload[0].payload.criterion}</p><p className="text-xs font-bold text-foreground">Level {formatProficiencyLevel(payload[0].value as number)}</p></div> : null} />
                        <Area type="monotone" dataKey="averageScore" stroke="hsl(var(--primary))" strokeWidth={3} fillOpacity={1} fill="url(#colorProgress)" />
                      </AreaChart>
                    )}
                  </ResponsiveContainer>
                </div>
                ...
                <div className="h-[220px] flex flex-col items-center justify-center text-center px-6">
                  <div className="h-12 w-12 bg-secondary/50 rounded-xl flex items-center justify-center mb-4"><Activity className="h-6 w-6 text-muted-foreground opacity-30" /></div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <Card className="border-none bg-secondary/20 dark:bg-primary/5 shadow-none overflow-hidden rounded-3xl">
              <CardHeader className="pb-2 px-8 pt-8">
                <div className="h-8 w-8 bg-primary/10 rounded-lg flex items-center justify-center mb-3"><Lightbulb className="h-4 w-4 text-primary" /></div>
                ...
                {insightData ? (
                  <>
                    <div className="space-y-3">
                      <p className="text-xs leading-relaxed text-foreground font-medium">Strongest performance in <span className="text-primary font-bold">{insightData.strongest} (Level {insightData.strongestLevel})</span>.</p>
                      <p className="text-xs leading-relaxed text-muted-foreground">{insightData.weakestLabel} are priority growth areas.</p>
                    </div>
                    <div className="pt-4 border-t border-border/50">
                      <div className="flex items-center gap-2 mb-2"><CheckCircle2 className="h-3.5 w-3.5 text-primary" /><p className="text-[9px] font-bold text-primary uppercase tracking-widest">Strategy</p></div>
                      <p className="text-xs italic text-muted-foreground leading-relaxed">&ldquo;{insightData.suggestion}&rdquo;</p>
                    </div>
                  </>
                ) : <div className="py-6 text-center"><p className="text-[10px] text-muted-foreground italic">Generating insights...</p></div>}
              </CardContent>
            </Card>

            <Card className="border-border bg-white dark:bg-[#111827] shadow-lg overflow-hidden rounded-3xl">
              <CardHeader className="bg-white dark:bg-[#111827] border-b border-border py-4 px-6"><CardTitle className="text-base font-bold text-foreground">Class Progress</CardTitle><CardDescription className="text-[9px] text-muted-foreground uppercase tracking-widest font-bold">Recent Scores</CardDescription></CardHeader>
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="space-y-0.5"><p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Avg. Proficiency</p><div className="flex items-baseline gap-1.5"><span className="text-2xl font-extrabold text-foreground">{classPerformance?.avgScore.toFixed(1) || '—'}</span><span className="text-[10px] font-bold text-muted-foreground">/ 8</span></div></div>
                  <div className="h-10 w-10 rounded-full border-2 border-secondary border-t-primary flex items-center justify-center"><span className="text-[10px] font-bold text-primary">{Math.round(((classPerformance?.avgScore || 0) / 8) * 100)}%</span></div>
                </div>
                  ...
                  {reports.slice(0, 2).map((report, i) => (
                    <div key={i} className="flex items-center justify-between p-2.5 rounded-xl bg-secondary/10 hover:bg-secondary/20 transition-colors cursor-pointer" onClick={() => router.push(`/teacher/reports/${report.reportId}`)}>
                      <div className="flex items-center gap-2">
                        <div className="h-6 w-6 rounded-md bg-white dark:bg-slate-800 flex items-center justify-center text-[9px] font-bold shadow-sm">{(report.studentName || 'S').charAt(0)}</div>
