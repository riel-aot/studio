  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-[#2F5BEA] p-px">
        <div className="relative overflow-hidden rounded-[0.95rem] bg-[#2F5BEA] px-8 py-10 flex items-center justify-between">
          <div className="relative z-10 max-w-xl space-y-3">
            <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">
              Welcome back, {user?.name || 'Teacher'}
            </h1>
            <p className="text-blue-100 text-base font-medium leading-relaxed max-w-md">
              You have {kpiData?.kpis.pendingReview ?? 3} assignments pending review. Check your queue to provide feedback.
            </p>
          </div>
          <div className="hidden lg:block relative h-40 w-56 mr-10">
            <Image 
      ...
      <div id="onboarding-kpis" className="space-y-4">
        <div className="flex items-center gap-3">
          <div className="h-7 w-7 rounded-full bg-blue-100 flex items-center justify-center">
            <Sparkles className="h-3.5 w-3.5 text-[#2F5BEA]" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-[#111827]">Today&apos;s Teacher Brief</h2>
            <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">Action Items</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-8 space-y-6">
          <Card id="onboarding-review-queue" className="border-[#E5E7EB] shadow-sm overflow-hidden">
            <CardHeader className="bg-white border-b border-[#F1F2F6] py-4">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-lg font-bold text-[#111827]">Grading Priority</CardTitle>
                  <CardDescription className="text-xs text-slate-500">Submissions ready for validation.</CardDescription>
                </div>
                <Badge variant="secondary" className="bg-blue-50 text-[#2F5BEA] border-none font-bold text-[10px]">
              ...
              {reviewQueueData?.items && reviewQueueData.items.length > 0 ? (
                <Table>
                  <TableHeader className="bg-slate-50/50">
                    <TableRow className="hover:bg-transparent border-b border-[#F1F2F6]">
                      <TableHead className="font-bold text-[#111827] h-10 text-xs">Student</TableHead>
                      <TableHead className="font-bold text-[#111827] h-10 text-xs">Assessment</TableHead>
                      <TableHead className="font-bold text-[#111827] h-10 text-xs">Status</TableHead>
                      <TableHead className="hidden md:table-cell text-right font-bold text-[#111827] h-10 text-xs">Last Update</TableHead>
                  ...
                  <TableBody>
                    {reviewQueueData.items.map((item) => (
                      ...
                      >
                        <TableCell className="font-semibold text-[#111827] py-3 text-sm">{item.studentName}</TableCell>
                        <TableCell className="text-slate-600 py-3 text-sm">{item.assessmentName}</TableCell>
                        <TableCell className="py-3">
                          <Badge variant={item.status === 'ai_draft_ready' ? 'default' : 'destructive'} className="rounded-md font-bold uppercase text-[9px] tracking-wider px-1.5 py-0 whitespace-nowrap">
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center px-6">
                  <div className="h-12 w-12 bg-blue-50 rounded-full flex items-center justify-center mb-3">
                    <Sparkles className="h-6 w-6 text-[#2F5BEA]" />
                  </div>
                  <h3 className="text-base font-bold text-[#111827]">Queue Cleared</h3>
                  <p className="text-xs text-slate-500 max-w-xs mt-1">All current submissions have been processed.</p>
                  <Button size="sm" onClick={() => startNewAssessment()} className="mt-4 bg-[#2F5BEA] font-bold">
          ...
          <Card className="border-[#E5E7EB] shadow-sm overflow-hidden">
            <CardHeader className="bg-white border-b border-[#F1F2F6] py-4">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg font-bold text-[#111827]">Class Health Snapshot</CardTitle>
              </div>
              <CardDescription className="text-xs text-slate-500">Real-time classroom engagement metrics.</CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-[10px] font-bold text-slate-400">Tgt: 90%</span>
                  </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card id="onboarding-quick-actions" className="bg-[#2F5BEA] text-white border-none shadow-lg shadow-blue-500/20 overflow-hidden relative">
            ...
            <CardHeader className="py-4">
              ...
              <CardDescription className="text-blue-100 text-xs">Common tasks.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-2 relative z-10">
              <Button size="sm" onClick={() => startNewAssessment()} className="w-full bg-white text-[#2F5BEA] hover:bg-blue-50 h-10 font-bold rounded-lg transition-all border-none text-xs">
                <FilePlus className="mr-2 h-4 w-4" /> New Assessment
              </Button>
              <Button asChild size="sm" variant="outline" className="w-full bg-transparent border-white/20 text-white hover:bg-white/10 h-10 font-bold rounded-lg text-xs">
                <Link href="/teacher/assessments"><PenSquare className="mr-2 h-4 w-4" /> All Assignments</Link>
              </Button>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-3">
              {draftsData?.items && draftsData.items.length > 0 ? (
                draftsData.items.map((draft) => (
                  <div key={draft.assessmentId} className="flex items-center justify-between group p-2.5 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer border border-transparent hover:border-slate-100">
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-[#111827] truncate">{draft.assessmentName}</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">{draft.studentName}</p>
                    </div>
                    <ChevronRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-[#2F5BEA] transition-colors shrink-0" />
                  </div>
                ))
              ) : (
                <p className="text-xs text-slate-400 text-center py-4">No active drafts.</p>
              )}
              <Button variant="ghost" className="w-full text-[#2F5BEA] font-bold hover:bg-blue-50 text-[10px] mt-1" asChild>
